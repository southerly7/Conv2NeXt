    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if lr_schedule_values is not None or wd_schedule_values is not None and data_iter_step % update_freq == 0:
            for i, param_group in enumerate(optimizer.param_groups):
                ...
                # if dp_schedule_values is not None:
                #     param_group["drop_path"] = dp_schedule_values[it]
        ...
        if use_amp:
            with torch.cuda.amp.autocast():
                ...
                if isinstance(output, list):
                    loss_list = [criterion(o, targets) / len(output) for o in output]
                    loss = sum(loss_list)
                else:
        ...
        #     min_dp = min(min_dp, group["drop_path"])
        #     max_dp = max(max_dp, group["drop_path"])
        #
        # metric_logger.update(dp=min_dp)
        # metric_logger.update(max_dp=max_dp)
        ...
        if log_writer is not None:
            ...
            # log_writer.update(dp=min_dp, head="opt")
            # log_writer.update(max_dp=max_dp, head="opt")
            if use_amp:
...
def evaluate(data_loader, model, device, use_amp=False):
    ...
    if isinstance(output, list):
        print(
            '* Acc@top1 {heads_top1.global_avg:.3f} Acc@top5 {heads_top5.global_avg:.3f} Acc@head_1 {head1_top1.global_avg:.3f} Acc@head_2 {head2_top1.global_avg:.3f} '
            'loss@total {losses.global_avg:.3f} loss@1 {loss_0.global_avg:.3f} loss@2 {loss_1.global_avg:.3f} '
            .format(heads_top1=metric_logger.acc1, heads_top5=metric_logger.acc5,
